...
class MixedOp(nn.Module):
  ...
  def __init__(self, C, stride):
    ...
    # *****************20210217*****************
    # The parameter 'ceil_model = TRUE' is becasue
    # STFT will give a odd dimension, while LFCC and LBF don't.
    # so originally, when use 'self.mp = nn.MaxPool2d(2,2)',
    # there will be a size mismatch when doing maxpooling in STFT,
    # but if padding=1, a mismatch will happen in LFCC/LBF (dimensions are even)
    ...
    for primitive in PRIMITIVES:
      ...
      if 'pool' in primitive:
        op = nn.Sequential(op, nn.BatchNorm1d(C//2, affine=False))
      self._ops.append(op)
...
class Network(nn.Module):
  ...
  def __init__(self, C, layers, args, num_classes, criterion, steps=4, multiplier=4, stem_multiplier=3):
